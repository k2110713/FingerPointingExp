...
# データを読み込む
file_path = "processed_results_with_throughput.csv"  # 適宜ファイルパスを設定
...
for (radius, method), group in data.groupby(['radius', 'method']):
    plt.plot(group['name'], group['throughput'], marker='o', label=f"Radius: {radius}, Method: {method}")

# グラフの設定
plt.xlabel("Name (Anonymized)", fontsize=12)
plt.ylabel("Elapsed Time (Mean)", fontsize=12)
plt.title("Individual Results of Throughput", fontsize=14)
